...
@_spi(Testing) public actor TargetDependencyResolver {
    ...
    private func addDependencies(forConfiguredTarget configuredTarget: ConfiguredTarget, toDependencyClosure dependencyClosure: inout OrderedSet<ConfiguredTarget>, dependencyPath: inout OrderedSet<ConfiguredTarget>, imposedParameters: SpecializationParameters? = nil) async {
        let nucleusDiagnosedTargets = ["Subprocess", "Subprocess-product", "SystemPackage", "SystemPackage-product"]
        ...
        if nucleusShouldDiagnose {
            let settings = buildRequestContext.getCachedSettings(configuredTarget.parameters, target: configuredTarget.target)
            print("NUCLEUS_SWIFTBUILD add target=\(configuredTarget.target.name) type=\(configuredTarget.target.type) configuredSDKROOT=\(settings.globalScope.evaluate(BuiltinMacros.SDKROOT).str) imposedSDKROOT=\(imposedParameters?.sdkRoot ?? "nil")")
        }
        ...
        let recordImmediateDependency: (ResolvedTargetDependency, SpecializationParameters?, inout OrderedSet<ConfiguredTarget>, inout OrderedSet<ConfiguredTarget>) async -> Void = { [self] dependency, imposedParameters, dependencyClosure, dependencyPath in
            ...
            if getEnvironmentVariable("NUCLEUS_SWIFTBUILD_DIAGNOSTICS") == "1"
                ...
                print("NUCLEUS_SWIFTBUILD edge parent=\(configuredTarget.target.name) dependency=\(dependency.target.target.name) type=\(dependency.target.target.type) configuredSDKROOT=\(settings.globalScope.evaluate(BuiltinMacros.SDKROOT).str) passedSDKROOT=\(imposedParameters?.sdkRoot ?? "nil") recursiveSDKROOT=\(dependencyImposedParameters?.sdkRoot ?? "nil")")
            }
